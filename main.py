...
def GOD(board, turn, movesMade, depth, alpha, beta):
            ...
            for j in range(len(board[i])):
                if board[i][j] and board[i][j].name > 0:
                    ...
                    for move in possibleMoves:
                        ...
                        score = GOD(tempBoard, turn*-1, movesMade+1, depth-1, alpha, beta)[0]
...
def inGame():
    ...
    while True:
        ...
        if turn == -1:
            board = copy.deepcopy(GOD(board, turn, movesMade, 2, float('-inf'), float('inf'))[1])
            movesMade += 1
            turn *= -1
